...
//	if(m==1e9+7 || m==998244353)
//	return binpowMod(a, b%(m-1), m);
	a%=m;
    long long res = 1;
    while (b > 0) {
        if (b & 1) //2^k
...
void solve() {
	...
	cout<<(4LL*binpow(3, ((N-4)/3), MOD))%MOD<<endl;
	else if(N%3==2)
		cout<<(2*binpow(3, ((N-2)/3), MOD))%MOD<<endl;
	else 
		cout<<binpow(3, N/3, MOD)%MOD<<endl;
