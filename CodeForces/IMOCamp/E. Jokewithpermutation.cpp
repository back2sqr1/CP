...
bool dfs(int i,int k)
{
    ...
    if ( a*10+ch[i+1]-'0'<=n&&a*10+ch[i+1]-'0'>9&&!vis[a *10+ch[i+1]-'0'])
    {
        ans[k]=(ch[i]-'0')*10+ch[i+1]-'0';
        vis[(ch[i]-'0')*10+ch[i+1]-'0']=1;
        if(dfs(i+2,k+1)) return true;
        vis[(ch[i]-'0')*10+ch[i+1]-'0']=0;
    }
    return false;
}
